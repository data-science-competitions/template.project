...
sources and analytic applications. The DAL has 3 stages with the following
functionality:
...
In addition, the template includes a data flow demonstration that implements all
three interfaces with a toy dataset.
...
plot_datasets_overview(
  list(
    historical_data = ingest_object$historical_data, 
    new_data = ingest_object$new_data,
    submission_sample = ingest_object$submission_sample
  )
)
```
...
```{r data-glimpse, eval=TRUE, message=FALSE, echo=FALSE}
ingest_object$historical_data %>% dplyr::glimpse(width = 100)
```
